...
- ラベル付け、スプリント割り当て、アサイン設定など、細かい設定値をAIに任せようとすると精度が悪く実用に耐えない

NotionにはModel Context Protocol (MCP)のサポートがありますが、読み取りは改善されてきた一方で、作成・更新系の操作はまだ使いづらく、CLIから直接書き込むような操作ができませんでした。
