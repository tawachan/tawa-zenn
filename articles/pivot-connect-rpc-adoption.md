...
HTTP/1.1 にネイティブ対応しているため、gRPC のように Envoy を立てて gRPC-Web を変換するといった仕掛けが不要です。既存の CDN やロードバランサーをそのまま活かせるため、インフラ面でのハードルが大きく下がりました。
...
Connect RPC は protobuf で通信しながら、必要に応じて JSON にフォールバックできます。エンジニア以外が動作確認したいときや、外部と一時的に連携するときに柔軟さがあり、導入当初に懸念していた「デバッグ体験が悪化する」問題は起こりませんでした。
